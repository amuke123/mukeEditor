...
function upload($file,$path,$type){
	...
	switch($type){
		case 'image':
			$ext_arr = array("jpg","jpeg","gif","png");break;
		case 'audio':
			$ext_arr = array("mp3","wma","rm","ogg");break;
		case 'video':
			$ext_arr = array("avi","mp4","ogg","mov");break;
		case 'file':
			$ext_arr = array("rar","zip","txt","pdf","docx","doc","xls","xlsx");break;
	...
	//检查目录
	if(@is_dir($save_path) === false){return "上传目录不存在。";}
	//检查目录写权限
	if(@is_writable($save_path) === false){return "上传目录没有写权限。";}
	//检查是否已上传
	if(@is_uploaded_file($tmp_name) === false){return "上传失败。";}
	//检查文件大小
